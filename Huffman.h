...
Node* makeNode(char, int, Node*, Node*); // Creates a new tree node with the character and its frequency
...
{
    ofstream out;
    out.open(Fl.c_str(), ios::trunc, ios::binary); //compressing the 1 and 0's from encoded file as they have greater size.
    int octets = Data.length() / 8;	//making new characters so that data is compressed and new file has less size.
    out.write((char*)&octets, sizeof(int)); // writing the character to the new file
...
string decompress(string Fl)
{
    ...
    char oc;
    while (in.read((char*)&oc, sizeof(char)))
    {
        Data = Data + oc;
    }
